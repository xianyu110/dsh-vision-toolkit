...
**所想即所见——给 DeepSeek Harness 里的纯文本 Agent 装上眼睛：看懂图片、定位界面、提取素材，并把视觉任务做到可验证。**

🚀 粘贴图片，直接提问 ｜ 一行命令安装即用 ｜ 内置免费额度 ｜ Web / Headless 无缝衔接
...
如果你在 DeepSeek Harness（DSH）里使用 DeepSeek 等纯文本模型，这些障碍可能并不陌生：

1. **图片粘贴不进去。** 一发图片就被拒绝，还要手动切换模型或复制文件路径。
2. **模型看不到关键内容。** 面对报错截图、网页或界面，只能靠你重新用文字描述。
3. **看图结果太笼统。** 普通图片描述缺少坐标、OCR 和任务上下文，做不了 UI 还原、长截图分析等复杂工作。
4. **接入门槛太高。** 安装后还要申请视觉模型、配置 API Key，不能马上体验。

## 亮点

- **粘贴即可使用。** 在 DSH Web 里粘贴图片，文本模型会自动切换到看图模式（`Vision Toolkit`）变体，不需要手动复制路径或更换模型。
- **无缝体验。** 图片保留原生缩略图、会话记录和工作区路径；Web 可以预览产物，无界面运行的 Headless 也能继续使用同一份结构化结果。
- **一行命令安装即用。** 安装插件后默认使用内置免费 Gemma 4 视觉服务，不需要申请 API Key。
- **内置免费额度。** 共享服务目前提供每客户端每天 100 次、全局每天 400 次和每 60 秒 20 次突发请求额度，并在触发限制时返回明确原因。
- **带着意图看图。** Agent 不只生成通用描述，而是围绕“报错在哪里”“按钮在哪”等当前任务提取证据。
- **结果可以继续工作。** 坐标、OCR、裁剪图、透明 PNG、SVG、热力图和 JSON 都能直接交给下一步。
- **从截图到可验证结果。** 参考图、HTML 截图、差异定位和像素对比组成一条完整的 UI 还原闭环。
